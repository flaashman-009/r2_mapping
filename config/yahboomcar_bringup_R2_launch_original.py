...
def generate_launch_description():
    ...
    yahboom_joy_node = Node(
        ...
        # 屏蔽原厂手柄节点的蜂鸣器输出。
        #
        # 原因：原厂 yahboom_joy_R2 把 buttons[11] 当成蜂鸣器开关，
        # 而我们用同一个键（buttons[11]）在 ps2_teleop 里做遥控解锁键。
        # 按一次解锁，原厂节点也跟着收到，于是往 /Buzzer 发 True，
        # 驱动端的 Buzzercallback 调 car.set_beep(1) —— 小车就一直响。
        #
        # 这里把它的 Buzzer 话题改名字，让它发不到驱动；
        # 解锁提示音改由 ps2_teleop 自己发（响三声就停）。
